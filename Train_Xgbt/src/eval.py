...
def main():
    ...
    ap.add_argument(
        "--mispred-csv",
        default=None,
        help="write rows with mismatched predictions to the specified CSV path",
    )
    args = ap.parse_args()
    ...
    print(f"F1 Score (test, threshold={args.threshold:.3f}): {f1:.6f}")

    if args.mispred_csv:
        mask = y_pred != y_true
        mispred_count = int(mask.sum())
        if mispred_count == 0:
            print("No mispredicted rows to output.")
        else:
            df_mis = df_te.loc[mask].copy()
            df_mis["predicted_proba"] = proba[mask]
            df_mis["predicted_label"] = y_pred[mask]
            df_mis["true_label"] = y_true[mask]
            df_mis.to_csv(args.mispred_csv, index=False)
            print(f"Wrote {mispred_count} mispredicted rows to '{args.mispred_csv}'.")
